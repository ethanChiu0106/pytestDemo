...
class TestProductPurchase:
    ...
    def test_purchase_a_product_successfully(self, page: Page, case: UIPurchaseCase):
        ...
        # 步驟 3: 瀏覽商品 (斷言)
        inventory_page.assert_url(InventoryPage.URL_REGEX, '驗證是否成功登入並跳轉到商品頁')
        ...
        inventory_page.assert_text(
            ...
            details_expected['quantity'],
            f'驗證購物車圖示數量為 {details_expected["quantity"]}',
        )

        # 步驟 5: 驗證購物車
        inventory_page.goto_shopping_cart_page()
        cart_page.assert_url(CartPage.URL_REGEX, '驗證是否進入購物車頁面')
        ...
        checkout_page.assert_url(CheckoutPage.STEP_ONE_URL_REGEX, '驗證是否進入結帳資訊頁面')
        ...
        checkout_page.assert_url(CheckoutPage.COMPLETE_URL_REGEX, '驗證是否進入結帳完成頁面')
        ...
        inventory_page.assert_url(InventoryPage.URL_REGEX, '驗證回到商品頁面')

...
@pytest.fixture
def password_change_session(pre_login: UserAPI, request):
    """
    ...
    yield pre_login  # 將登入後的 UserAPI client 提供給測試
    ...
    if case and 'change_password_success' in request.node.callspec.id:
        ...
        # pre_login 就是已登入的 UserAPI client
...
class TestChangePassword:
    ...
    def test_change_password(
        self, password_change_session: UserAPI, case: ChangePasswordCase, setup_change_password_user
    ):
        user_api = password_change_session
        request = case.request
        excepted = case.expected
        old = request.old_password
        new = request.new_password
        actual_result = user_api.change_password(old, new)
